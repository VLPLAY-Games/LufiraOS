 ...
 * Mass Storage. Заменяет прежний UHCI-драйвер; вместо Frame List из
 * Queue Head/TD здесь Command Ring, Event Ring и per-устройство
 * Device/Input Context. Прерывания контроллера не используются (в этом
 * ядре нет MSI-X/PCI capability list) — Event Ring опрашивается напрямую
 * из usb_poll() раз в тик PIT.
 */
