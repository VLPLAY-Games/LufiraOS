...
// Выделяет страницу и маппит её в УКАЗАННОМ адресном пространстве (по
// pml4_phys), не переключая CR3 — доступ к таблицам идёт через identity
// mapping. Вызывающий обязан гарантировать, что текущий CR3 позволяет
// читать/писать все нужные физические адреса, и что pml4_phys валиден.
                     ...
                     char *argv[], char *envp[])
{
    ...
    // process_commit_exec() перезапишет proc->page_table — иначе указатель
    // теряется и exec() навсегда утекал бы старый образ процесса. Свободить
    // можно уже сейчас, до переключения CR3: free_user_address_space()
    // работает через physmap, не через текущий активный CR3.
