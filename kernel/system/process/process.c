        ...
        for (int pdpt_idx = 0; pdpt_idx < 512; pdpt_idx++) {
            ...
            for (int pd_idx = 0; pd_idx < 512; pd_idx++) {
                ...
                if (src_pd[pd_idx] & PAGE_PS) {
                    // Пользовательские маппинги в этой кодовой базе
                    // huge-страницами не создаются (map_page_in_space
                    // всегда работает по 4KB) — НО ровно это же место в
                    // PML4[0] занимает разделяемый identity-map ядра
                    // (build_identity_pdpt() в paging.c, huge 2MB-страницы;
                    // см. clone_low_identity_map()). Раньше здесь стоял
                    // continue ("пропускаем — не бывает"), и запись просто
                    // терялась: у ребёнка после fork() дыра ровно там, где
                    // должен быть маппинг кода САМОГО ЯДРА (включая
                    // context_switch()) — CR3 планировщик переключал на
                    // клон нормально, а вот СЛЕДУЮЩАЯ же выборка инструкции
                    // ядра тут же не находила страницу и валила систему в
                    // page fault -> тройной fault, ещё до первой инструкции
                    // ребёнка. Ядерная физическая память общая для всех
                    // процессов и не копируется по-настоящему нигде (даже
                    // clone_low_identity_map() просто копирует ту же самую
                    // 2MB-запись) — так что здесь достаточно скопировать
                    // запись по значению, как и для остального identity-map.
