...
//
// Этот заголовок сознательно не включает НИЧЕГО, кроме <stdint.h> — его
// подключают ДВЕ РАЗНЫЕ стороны: сам kernel (freestanding-сборка) и
// хостовый инструмент форматирования tools/mkfs_lufirafs.c (обычная сборка
// с libc). Обе стороны обязаны видеть ОДИНАКОВЫЕ структуры/константы/
// арифметику раскладки — расхождение здесь означает "ядро не понимает то,
// что записал mkfs" без единого шанса поймать это на этапе компиляции.
//
...
// UEFI-прошивка умеет читать файлы ТОЛЬКО с FAT12/16/32 — это требование
// самой спецификации UEFI, встроенный в прошивку драйвер другого просто не
// понимает. Поэтому первые LUFIRAFS_ESP_SIZE байт диска остаются маленьким
// FAT-разделом (ESP), где лежат ИСКЛЮЧИТЕЛЬНО /EFI/BOOT/BOOTX64.EFI и
// /kernel.bin — их читает сама прошивка и бутлоадер до появления этого
// драйвера. Всё остальное место на диске (данные, пользовательские файлы)
// занимает LufiraFS. Загрузчик (boot/loaders/fat_loader.c) грузит в RAM
// ВЕСЬ диск одним куском начиная с LBA 0 — то есть bi->FATImageBase уже
// содержит и ESP, и регион LufiraFS; ядру достаточно просто прибавить это
// смещение (см. kernel.c). Значение общее для kernel И tools/mkfs_lufirafs —
// расхождение здесь означает, что mkfs отформатирует не тот регион, что
// потом читает ядро.
